...
jvm.$ = jQuery, jvm.AbstractElement = function(a, b) {
...
}, jvm.inherits(jvm.SVGCircleElement, jvm.SVGShapeElement), jvm.VMLElement = function(a, b) {
	...
	if(this.width = a, this.height = b, this.node.style.width = a + "px", this.node.style.height = b + "px", this.node.coordsize = a + " " + b, this.node.coordorigin = "0 0", this.rootElement) {
		for(c = this.rootElement.node.getElementsByTagName("shape"), e = 0, f = c.length; e < f; e++) c[e].coordsize = a + " " + b, c[e].style.width = a + "px", c[e].style.height = b + "px";
		for(d = this.node.getElementsByTagName("group"), e = 0, f = d.length; e < f; e++) d[e].coordsize = a + " " + b, d[e].style.width = a + "px", d[e].style.height = b + "px"
	}
	...
	switch(a) {
		...
		case "stroke-opacity":
			...
			break;
		case "stroke-width":
			0 === parseInt(b, 10) ? this.node.stroked = !1 : this.node.stroked = !0, this.node.strokeweight = b;
			break;
		case "d":
	...
	var b, c, d = 0,
		e = 0;
	return a = a.replace(/(-?\d+)e(-?\d+)/g, "0"), a.replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function(a, f, g, h) {
	...
	return this.mode = window.SVGAngle ? "svg" : "vml", "svg" == this.mode ? this.impl = new jvm.SVGCanvasElement(a, b, c) : this.impl = new jvm.VMLCanvasElement(a, b, c), this.impl
	...
	setValues: function(a) {
		var b, c, d = Number.MIN_VALUE,
			e = Number.MAX_VALUE,
			f = {};
		if(this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale)
			for(c in a) a[c] ? f[c] = this.scale.getValue(a[c]) : f[c] = this.elements[c].element.style.initial[this.params.attribute];
		else {
			if(!this.params.min || !this.params.max) {
				for(c in a) b = parseFloat(a[c]), b > d && (d = a[c]), b < e && (e = b);
				this.params.min || this.scale.setMin(e), this.params.max || this.scale.setMax(d), this.params.min = e, this.params.max = d
			}
			for(c in a) b = parseFloat(a[c]), isNaN(b) ? f[c] = this.elements[c].element.style.initial[this.params.attribute] : f[c] = this.scale.getValue(b)
		}
	...
	bindElementEvents: function() {
		...
		this.container.mousemove(function() {
			...
			var c = jvm.$(this).attr("class").baseVal ? jvm.$(this).attr("class").baseVal : jvm.$(this).attr("class"),
				d = -1 === c.indexOf("jvectormap-region") ? "marker" : "region",
				e = "region" == d ? jvm.$(this).attr("data-code") : jvm.$(this).attr("data-index"),
				f = "region" == d ? b.regions[e].element : b.markers[e].element,
				g = "region" == d ? b.mapData.paths[e].name : b.markers[e].config.name || "",
				h = jvm.$.Event(d + "LabelShow.jvectormap"),
				i = jvm.$.Event(d + "Over.jvectormap");
		...
		}), this.container.delegate("[class~='jvectormap-element']", "mousedown", function(b) {
			a = !1
			...
			var d = jvm.$(this).attr("class").baseVal ? jvm.$(this).attr("class").baseVal : jvm.$(this).attr("class"),
				...
				g = jvm.$.Event(e + "Click.jvectormap"),
				h = "region" == e ? b.regions[f].element : b.markers[f].element;
			a || (b.container.trigger(g, [f]), ("region" === e && b.params.regionsSelectable || "marker" === e && b.params.markersSelectable) && (g.isDefaultPrevented() || (b.params[e + "sSelectableOne"] && b.clearSelected(e + "s"), h.setSelected(!h.isSelected))))
		})
	},
	...
	createLabel: function() {
		...
		this.label = jvm.$("<div/>").addClass("jvectormap-label").appendTo(jvm.$("body")), this.container.mousemove(function(b) {
			...
			c < 5 && (c = b.pageX + 15), d < 5 && (d = b.pageY + 15), a.label.is(":visible") && a.label.css({
				left: c,
				top: d
			})
		})
	...
	setFocus: function(a, b, c) {
		...
		if(jvm.$.isArray(a) || this.regions[a]) {
			for(g = jvm.$.isArray(a) ? a : [a], h = 0; h < g.length; h++) this.regions[g[h]] && (e = this.regions[g[h]].element.getBBox()) && (void 0 === d ? d = e : (f = {
				x: Math.min(d.x, e.x),
				y: Math.min(d.y, e.y),
	...
	createMarkers: function(a) {
		...
		if(this.markersGroup = this.markersGroup || this.canvas.addGroup(), jvm.$.isArray(a))
			for(f = a.slice(), a = {}, b = 0; b < f.length; b++) a[b] = f[b];
		for(b in a) e = a[b] instanceof Array ? {
			latLng: a[b]
		} : a[b], !1 !== (d = this.getMarkerPosition(e)) && (c = this.canvas.addCircle({
			"data-index": b,
			cx: d.x,
			cy: d.y
	...
	latLngToPoint: function(a, b) {
		var c, d, e, f = jvm.WorldMap.maps[this.params.map].projection,
			g = f.centralMeridian;
		this.width, this.baseTransX, this.baseScale, this.height, this.baseTransY, this.baseScale, this.scale, this.baseScale;
		return b < -180 + g && (b += 360), c = jvm.Proj[f.type](a, b, g), !!(d = this.getInsetForPoint(c.x, c.y)) && (e = d.bbox, c.x = (c.x - e[0].x) / (e[1].x - e[0].x) * d.width * this.scale, c.y = (c.y - e[0].y) / (e[1].y - e[0].y) * d.height * this.scale, {
			x: c.x + this.transX * this.scale + d.left * this.scale,
			y: c.y + this.transY * this.scale + d.top * this.scale
		})
	},
	pointToLatLng: function(a, b) {
		var c, d, e, f, g, h = jvm.WorldMap.maps[this.params.map].projection,
			i = h.centralMeridian,
			j = jvm.WorldMap.maps[this.params.map].insets;
	...
	createSeries: function() {
		...
		for(b in this.params.series)
			for(a = 0; a < this.params.series[b].length; a++) this.series[b][a] = new jvm.DataSeries(this.params.series[b][a], this[b])
	},
	remove: function() {
